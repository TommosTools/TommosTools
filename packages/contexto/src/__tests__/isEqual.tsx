...
// eslint-disable-next-line @typescript-eslint/comma-dangle, object-curly-newline
const Render = <T,>({ context, testId, isEqual, callback }: RenderProps<T>) =>
{
	callback?.();
	...
	const Component = () =>
	{
		...
		return (
			<Context1.Provider value={val}>
				<Button testId="ClickTarget" onClick={() => setVal("ghi")} label="click" />
...
test("isEqual compares previous rendered value rather than previous updated value", async () =>
{
	...
	const Component = () =>
	{
		...
		return (
			<Context1.Provider value={val}>
				<Button testId="ClickTarget1" onClick={() => setVal(1)} label="click" />
				<Button testId="ClickTarget2" onClick={() => setVal(2)} label="click" />
		...
		return (
			<Context1.Provider value={val1}>
				<Context2.Provider value={val2}>
					<Button testId="ClickTarget1" onClick={() => setVal1("ghi")} label="click" />
					<Button testId="ClickTarget2" onClick={() => setVal2((val) => val + 1)} label="click" />
					<UseContextsComponent />
				</Context2.Provider>
			</Context1.Provider>
		);
